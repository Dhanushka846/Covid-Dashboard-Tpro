...
def get_historic_cases(country, days):
  ...
  resp = requests.get(f'https://disease.sh/v3/covid-19/historical/{country}?lastdays={days}')
  data = resp.json()
...
def get_yesterday_cases(country):
  ...
  resp = requests.get(f'https://disease.sh/v3/covid-19/historical/{country}?lastdays=1')
  data = resp.json()
